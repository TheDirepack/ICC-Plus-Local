...
def run(*args:str)->None:
    env=os.environ.copy(); env['PYTHONPATH']=str(TOOL_ROOT)
    subprocess.run([sys.executable,'-m','iccplus_tools',*args],check=True,cwd=HERE,env=env)
def main()->None:
    BUILD.mkdir(exist_ok=True); STATE.unlink(missing_ok=True)
