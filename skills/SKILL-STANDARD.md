...
Keep installable skill directories flat under `/CYOA/skills/`; do not create category folders around them.
...
The remaining high-level flow is `develop → plan → migrate if needed → iccplus-local → review → ship`. `cyoa-compress` remains only for exceptional external or legacy asset compression. `unslop` is a shared cross-project skill and lives only at `/skills/unslop`.

Do not recreate `cyoa-create`, `cyoa-edit`, `cyoa-look`, or `cyoa-test`. Their old workflows are now covered by `iccplus-local`.

Keep per-skill details in that skill's `references/` folder. Put project-wide guide material in `/CYOA/cyoa-guide/` and reusable executable tools in `/CYOA/tools/`. Do not copy a shared global skill into `/CYOA/skills/`; reference its canonical `/skills/<name>/` location instead.
...
Project-level documentation outside the skill directory may be referenced by its stable project path, such as `/CYOA/cyoa-guide/...`.
