...
- Global `/skills/unslop` handles non-code prose cleanup.
...
Read `/CYOA/skills/iccplus-local/SKILL.md`, then only the function guide needed for the current operation under `/CYOA/skills/iccplus-local/functions/`.
...
Start with `/CYOA/cyoa-guide/00-index.md` for design and implementation guidance.

Detailed local-tool documentation and release metadata live under `/CYOA/tools/iccplus-local/`.
...
Shared cross-project skills belong under `/skills/`. Keep `/CYOA/skills/` for CYOA-specific skills only.