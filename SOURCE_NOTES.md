...
The returned official-GUI verification material includes one strong Creator result: the C01 Save to Disk artifact exactly matches the pinned 2.10.7 round-trip fixture. Other returned cases provide partial evidence but do not complete the current structured browser-verification gate. The local verifier therefore keeps official Creator/Viewer verification separate from the local regression suite.
